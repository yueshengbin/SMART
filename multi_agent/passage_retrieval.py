...
class Retriever:
    ...
    def add_passages(self, passages, top_passages_and_scores):
        ...
        docs = [passages[doc_id] for doc_id in top_passages_and_scores]
        return docs
    ...
    def search_document(self, query, top_n=10):
        ...
        for item in query:
            queries_list.append(item["instruction"] + "\n" +item["output"])
        questions_embedding = self.embed_queries(self.args, queries_list)
        ...
        for i in range(len(queries_list)):
            for cont in query:
                contex = self.add_passages(self.passage_id_map, top_ids_and_scores[i][0])[:top_n]
                cont['retrieval'] = contex
        return query
...
def main(args):
    ...
    queries = load_data(args.dataset)
    docum= retriever.search_document(queries, args.n_docs)
    with open(args.output_dir, 'w',encoding="utf-8") as file:
        json.dump(docum, file, indent=4,ensure_ascii=False)
